...
module.exports = {
    ...
    LEAVE_REQUEST:async (req,res)=>{
        ...
        if(companyEmployee != null)
        {
            ...
            .then(async (result) => {
                let company = await PROVIDER.COMPANY.COMPANY_BY_COMPANY_ID(companyEmployee.companyId)
                let ownUser = company.ownUser
                let profile = await PROVIDER.EMPLOYEE_PROFILE.EMPLOYEE_BY_USERID(User.id)
                SEND_TELEGRAM_NOTIFICATION(ownUser,profile.name+" - Leave request is sent.")
