        ...
        if(isUser.password == null)
        {
            ...
            .then(async (result) => {
                if(result[0] != 0)
                {
                    ...
                    let company = await PROVIDER.COMPANY.COMPANY_BY_COMPANY_ID(pendingUser.company)
                    let ownUser = company.ownUser
                    SEND_TELEGRAM_NOTIFICATION(ownUser,`${pendingUser.firstName} successfully creates employee profile!`)
                }
                else
